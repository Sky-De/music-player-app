import { useNavigate } from "react-router-dom";

const artistAvatarUrl = 'data:image/png;base64,';
const ArtistSongCard = ({ song, i }) => {
  const navigate = useNavigate();
    return(
    <div onClick={() => navigate(`/search/${song.name}`)} 
     className="w-full flex flex-row items-center
     hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2">
     <h4 className="text-white font-bold text-base mr-3">{i + 1}. </h4>
     <div className="flex-1 flex justify-between items-center">
      <img className="w-16 h-16 rounded-lg" src={ artistAvatarUrl } alt={song.albumName} />
      <div className="flex-1 flex flex-col justify-center mx-3">
          <p className="text-xl font-bold text-white w-[250px] sm:w-auto  truncate">{song.name}</p>
          <p className="text-base font-semibold text-gray-300 w-[250px] sm:w-auto  truncate">{song.albumName}</p>
      </div>
     </div>
   </div>
  )}


const ArtistSongs = ({ data }) => {
  
  return(
  <div 
   className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[650px] md:max-w-[80vw] max-w-[90vw] flex flex-col select-none">
    <div className="w-full flex flex-col">
      <div className="flex flex-row justify-between items-center">
        <h2 className="text-white font-bold text-2xl">Songs</h2>
      </div>

      <div className="mt-4 flex flex-col gap-1">
        {Object.values(data?.resources.songs).map((song,i) => (
          <ArtistSongCard 
           key={song.id} 
           song={song.attributes} 
           i={i}
           />
        ))}
      </div>
    </div>
   </div>
);
}

export default ArtistSongs;
