...
const ArtistDetails = () => {
  ...
  if (isFetching) return <Loader title="Loading artist data..." />;
        ...
        <div className="absolute inset-0 flex items-center">
          ...
          <div className="ml-5 select-none">
            <p className="text-xl font-bold text-white sm:text-3xl">
              {Object.values(data?.resources.artists)[0].attributes.name}
            </p>
            {Object.values(
              data?.resources.artists
            )[0].attributes.genreNames.map((genre, i) => (
              <p key={i} className="mt-2 text-base text-red-400">
                {genre}
              </p>
            ))}
          </div>
