...
const SongDetails = () => {
  ...
  const {
    ...
    isFetching: isFetchingArtistTopSongsData,
    error: relatedError,
    ...
    <div className="flex flex-col">
      ...
      <div className="mb-10">
        <h2 className="text-3xl font-bold text-white">
          Lyrics for <span className="text-red-400">{songData?.title}</span> :{" "}
        </h2>
        <div className="mt-5">
          {songData?.sections[1].type === "LYRICS" ? (
            songData?.sections[1].text.map((line, i) => (
              <p key={i} className="my-1 text-base text-gray-400">
                {line}
              </p>
            ))
          ) : (
            <p className="my-1 text-base text-gray-400">No lyrics found...!</p>
          )}
        </div>
      </div>
